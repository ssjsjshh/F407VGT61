...
#include <string.h>
/* ---------------------------1KB缓存刷新----------------------------------------------- */
static uint8_t OLED_GRAM[OLED_PAGES][OLED_WIDTH]={0};//1KB缓存刷新
...
/* ---------------------------OLED发送单字节命令----------------------------------------------- */
static void OLED_WR_Cmd(uint8_t dat)
{
    OLED_CS_L();
    OLED_DC_L();
...
/* ---------------------------OLED连续发送数据----------------------------------------------- */
static void OLED_WR_Dat_Burst(const uint8_t *buf,uint8_t len)
{
    if(len==0||buf==NULL)
    return;
    OLED_CS_L();
...
/* ---------------------------OLED连续写入固定值----------------------------------------------- */
static void OLED_Fill_Burst(uint8_t dat,uint8_t len)
{
    if(len == 0)return;
    OLED_CS_L();
    OLED_DC_H();
...
/* ---------------------------OLED刷新显示----------------------------------------------- */
void OLED_Show(uint8_t x,uint8_t y,const char* str,uint8_t size)//OLED显示
{
    uint8_t i=0,ch=0;
    switch(size) 
    {
